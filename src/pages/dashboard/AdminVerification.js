...
export default function AdminVerification() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/verification/admin/all`)
      .then((res) => res.json())
  ...
  async function handleApprove(id) {
    try {
      const res = await fetch(`${BASE_URL}/api/verification/${id}/approve`, { method: "PUT" });
  ...
  async function handleReject(id) {
    try {
      const res = await fetch(`${BASE_URL}/api/verification/${id}/reject`, { method: "PUT" });
      if (!res.ok) throw new Error();
  ...
  if (role !== "admin") {
    return (
      <DashboardLayout title="Verify Drivers">
        <div className="section">
          <h3 className="section-title">Admin only</h3>
          <div className="section-sub">Switch role to Admin to verify drivers.</div>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout title="Verify Drivers">
      <div className="section">
        <div className="section-head">
          <div>
            <h3 className="section-title">Driver Verifications</h3>
            <div className="section-sub">Approve or reject driver submissions.</div>
          </div>
        ...
        {!loading && verifications.length === 0 && (
          <div className="p-muted">No driver submissions yet.</div>
        )}
              ...
              {expanded === v.id && (
                <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginTop: 16 }}>
                  {[
                    ...
                    { label: "License", path: v.licensePhotoPath },
                    { label: "Vehicle RC", path: v.vehicleRcPhotoPath },
